...
export const payloadSeoPlugin = (pluginOptions: SeoPluginConfig) => (
  ...
  const generateMetaDescriptionFunction = getGenerateMetaDescription(pluginOptions, incomingConfig)

  const officialSeoPlugin = seoPlugin({
    ...pluginOptions,
    generateDescription: generateMetaDescriptionFunction,
    fields: ({ defaultFields }) => [keywordsField(), imageField(), ...defaultFields],
  })
  ...
  config.onInit = async (payload) => {
    ...
    const neededEnvVars = ['DEEPSEEK_API_KEY']
